...
(function () {
	...
	function completeSrc( src ) {
		return /\.js$|\.json$/.test( src ) ? src : src + ".js";
	}

	// 获取当前正在执行脚本的src,如果正在执行的是个内联脚本,返回""
	function currentScriptSrc() {
		var curScript = document.currentScript || document.currentScript || document.scripts[document.scripts.length - 1];
		return curScript.getAttribute( "src" ) || "";
	}

	// 加了时间戳的src,用于防缓存
	function StampSrc( src ) {
		return src + ( navigator.platform !== "Win32" ? "?t=" + new Date().getTime() : "" );
	}
	...
	function getRequires( func, done, baseUri ) {
		...
		while ( ( regResult = requirePattern.exec( code ) ) !== null ) {
			...
			absUrl = toAbsURL( trimSrc( /\//.test( moduleName ) ? moduleName : join( coreLibBaseURI, moduleName ) ) );
		...
		function load( index ) {
			...
			if ( src ) {
				if ( /\.json$/.test( src ) ) {
					...
					xhr.open( "get", StampSrc( src ), true );
					xhr.onload = function () {
						lib[toAbsURL( src )] = JSON.parse( xhr.responseText );
						load( index + 1 );
					};
					xhr.send( null );
				}
