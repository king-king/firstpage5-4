...
function html( arg ) {
	...
	// 添加meta
	array.foreach( arg.meta, function ( meta ) {
	...
	return jsdom.serializeDocument( document ).replace( "<script></script>", "<script>" + arg.script + "</script>" );
}
...
async.sequence( [
		...
		async.sequence( [
			...
			function ( callback ) {
				zfs.readJSONFile( "../application.json", goOn( function ( result ) {
					var js = result.js;
					application = result;
					application.js = ["out.js"].concat( js );
					application.pageList = ["../page-list/page-list-out.js"].concat( js );
					callback();
				} ) );
			},
			...
			// 合并脚本
			function () {
				function mergeJS( target, callback ) {
					merge.mergeJS( {
						input : [{
							path : "",
				...
				async.concurrency( [
					function ( done ) {
						mergeJS( applicationContentList.js, function ( result ) {
							project.push( {
					...
					function ( done ) {
						mergeJS( applicationContentList.pageList, function ( result ) {

							yui.compressString( result, {
								type : "js"
							}, goOn( function ( result ) {
								project.push( {
									path : "content/page-list.html",
									data : html( {
										title : "页面列表",
										script : result,
										meta : [
	...
	// 复制其他content文件
	function ( callback ) {
		zfs.listDir( "../content", function ( err, result ) {
			async.concurrency( array.map( array.remove( result, function ( content ) {
				return !/^\.\./.test( Path.relative( "../content/image", content.path ) );
			} ), function ( content ) {
				return function ( done ) {
					zfs.readContent( content, function ( err, result ) {
						project.push( {
							path : Path.relative( "..", result.path ),
							data : result.data
						} );
						done();
					} );
